...
public class MapGenerator : MonoBehaviour
{
    ...
    void SpawnPrefeb()
    {
		...
		//player.GetComponent<PlayerMovement>().pTrans.position = playerLoc;
	}
	...
	void Divide(Node tree, int n)
	{
		if (n == maximumDepth)
		{
			playerLoc = new Vector2((tree.nodeRect.x) / 2, (tree.nodeRect.y) / 2);
	...
	private void GenerateLoad(Node tree, int n)
	{
		...
		{
			tileMap.SetTile(new Vector3Int(i - mapSize.x / 2, leftNodeCenter.y - mapSize.y / 2), roomTile);
		}

		for (int j = Mathf.Min(leftNodeCenter.y, rightNodeCenter.y); j <= Mathf.Max(leftNodeCenter.y, rightNodeCenter.y); j++)
		{
			tileMap.SetTile(new Vector3Int(rightNodeCenter.x - mapSize.x / 2, j - mapSize.y / 2), roomTile);
		}
	...
	void FillBackground() //배경을 채우는 함수, 씬 load시 가장 먼저 해준다.
	{
												   ...
												   //맵 크기보다 넓게 채워준다.
		{
			...
			{
				tileMap.SetTile(new Vector3Int(i - mapSize.x / 2, j - mapSize.y / 2), outTile);
			}
		}
	}
	void FillWall() //룸 타일과 바깥 타일이 만나는 부분
	{
		for (int i = 0; i < mapSize.x; i++) //타일 전체를 순회
		{
			...
			{
				if (tileMap.GetTile(new Vector3Int(i - mapSize.x / 2, j - mapSize.y / 2)) == outTile)
				{
					//바깥타일 일 경우
					for (int x = -1; x <= 1; x++)
					{
						for (int y = -1; y <= 1; y++)
						{
							...
							if (tileMap.GetTile(new Vector3Int(i - mapSize.x / 2 + x, j - mapSize.y / 2 + y)) == roomTile)
							{
								tileMap.SetTile(new Vector3Int(i - mapSize.x / 2, j - mapSize.y / 2), wallTile);
								break;
							}
						}
					}
	...
	{ //room의 rect정보를 받아서 tile을 set해주는 함수
		for (int i = rect.x; i < rect.x + rect.width; i++)
		{
			...
			{
				tileMap.SetTile(new Vector3Int(i - mapSize.x / 2, j - mapSize.y / 2), roomTile);
			}
		}
	}
}
